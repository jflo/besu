...
class SigParamOperationTest {

  private final SigParamOperation operation = new SigParamOperation(new PragueGasCalculator());
  private final EVM fakeEVM = mock(EVM.class);

  @Test
  void validSigIndexReturnsZeroForNow() {
    MessageFrame frame = mock(MessageFrame.class);
    when(frame.popStackItem())
        .thenReturn(Bytes.of(0)) // sigIndex
        .thenReturn(Bytes.of(SigParamOperation.PARAM_SIGNER)); // param
