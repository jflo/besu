...
class FrameDataCopyOperationTest {

  private final FrameDataCopyOperation operation =
      new FrameDataCopyOperation(new PragueGasCalculator());
  private final EVM fakeEVM = mock(EVM.class);

  @Test
  void validFrameIndexSucceeds() {
    MessageFrame frame = mock(MessageFrame.class);
    when(frame.popStackItem())
        .thenReturn(Bytes.of(0)) // frameIndex
        .thenReturn(Bytes.of(0)) // dataOffset
        .thenReturn(Bytes.of(32)) // length
        .thenReturn(Bytes.of(0)); // memOffset
  ...
  void outOfBoundsFrameIndexHalts() {
    MessageFrame frame = mock(MessageFrame.class);
    when(frame.popStackItem())
        .thenReturn(Bytes32.repeat((byte) 0xFF)) // huge frameIndex
