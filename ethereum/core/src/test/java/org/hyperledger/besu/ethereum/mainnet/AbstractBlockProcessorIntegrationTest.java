...
class AbstractBlockProcessorIntegrationTest {
  ...
  // EIP-2935 history contract. It is not deployed in this genesis, but the block-start system call
  // still reads its account before finding there is no code to run, so EIP-7928 records it — with
  // no
  // changes — in every block's access list.
  private static final Address HISTORY_CONTRACT =
